...
- BackupStorageLocation Controller 周期性检查 BackupStorageLocation 是否可用时
...
- BackupSync Controller 周期性同步集群和 BackupStorageLocation 中的 Backup 时
...
- Restore Controller 在恢复时，会将备份的内容下载到临时文件中
...
- BackupSync Controller 在获取到需要同步 Backup 时，如果 Velero 开启了 CSI 特性，也会同步 VolumeSnapshotContents 对象
...
- BackupDeletion Controller 删除指定备份时
...
- Restore Controller 在恢复完成时，会将本次恢复过程中产生的 warning 和 error 信息上传
...
- BackupDeletion Controller 在删除 Backup 时，会同步删除相关联的 Restore<br>*BackupStorageLocation 中 Restore 的删除仅在该场景下，手动删除 Restore，不会触发 DeleteRestore 流程，也就是通过 velero restore delete xxx 是无法删除 BackupStorageLocation 中 Restore 文件的。*
...
- DownloadRequest Controller 在处理 DownloadRequest 对象时
