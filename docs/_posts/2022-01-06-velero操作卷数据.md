...
*velero-restic-restore-helper init container*

```yaml
initContainers:
- args:
  - 4d9a0fa5-5936-4d02-8597-b19f1b92cabc
  command:
  ...
  volumeMounts:
  - mountPath: /restores/local-pvc
    name: local-pvc
  - mountPath: /var/run/secrets/kubernetes.io/serviceaccount
    name: default-token-qknv2
    readOnly: true
```
...
与此同时，每个节点的 restic controller 会有一个 `/var/lib/kubelet/pods` 的 hostPath 卷挂载用来访问 Pod 的卷数据，等待 Pod 运行 init 容器，通过访问该 hostPath 卷，获取到待备份的 Pod 卷数据后，执行 `restic restore`，成功之后，将文件写入 Pod 卷中的 .velero 子目录中，名称是 restore 任务的 UID，并根据实际情况将 PodVolumeRestore 状态设置为 completed 或者 failed。
