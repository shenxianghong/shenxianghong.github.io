...
- 将相关字段更新至集群中的 BackupStorageLocation 对象中，如果设置其为默认， 会将集群中其余的 BackupStorageLocation 设为非默认
...
3. 通过 StorageProvider 的 ListCommonPrefixes 接口判断 BackupStorageLocation 是否可用<br>*具体规则是去除 prefix 目录层级，获取下一个目录层级，该目录层级应为 backups、restores、restic、metadata 和 plugins 中之一。详细逻辑参考 [TestIsValid](https://github.com/vmware-tanzu/velero/blob/5fe3a50bfddc2becb4c0bd5e2d3d4053a23e95d2/pkg/persistence/object_store_test.go#L70)*
...
3. 否则会进一步判断 ResticRepository 对象状态，如果状态为 Ready，则执行 restic prune 命令，并更新 ResticRepository 上次维护时间信息；如果状态为 NotReady，调用 **ensureRepo** 尝试检查或初始化一个仓库，并根据调用结果更新 ResticRepository 的状态为 Ready 或者 NotReady<br>*restic prune 的执行失败并不会影响到主流程，只是会在 ResticRepository 对象中记录错误信息*
...
2. 如果命令执行返回错误信息中包含 “Is there a repository at the following location?”  字符串，表示 Restic repo 不存在，会通过 restic init 命令初始化一个新仓库
