...
- 在指定 --backup-sync-period 时，值必须要大于等于 0 
- 在指定 --credential 时，值仅能有一对 key-value

**流程逻辑**

1. 根据命令行参数，构建 BackupStorageLocation 对象，在集群中创建，后续由 `BackupStorageLocation Controller` 负责维护
...
- 要获取的 BackupStorageLocation 在集群中必须存在
...
- 要获取的 ResticRepository 在集群中必须存在
