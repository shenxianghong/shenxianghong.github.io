...
- name、--all 和 --selector 参数有仅能有一个
- 要删除的 BackupStorageLocation 在集群中必须存在

**流程逻辑**

1. 获取集群中的 BackupStorageLocation 资源并删除
...
2. 如果 ResticRepository 对象状态为空或者是 New，则调用 **initializeRepo** 初始化一个 Restic 仓库并返回；否则会进一步判断 ResticRepository 对象状态：
...
2. 将全量 ResticRepository 对象加入到 Generic Controller 的 queue 中<br>*之所以是全量，是因为网络连接的不确定性，需要重新判断所有的 ResticRepository 可达状态*
