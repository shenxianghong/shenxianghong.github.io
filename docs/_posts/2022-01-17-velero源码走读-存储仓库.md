...
删除备份任务时，根据 `bsl name`，`--all` 或者 `--selector` 获取到集群中的 BackupStorageLocation 资源并删除。(*删除 BackupStorageLocation 时，并不会删除集群中相关联的 Backup。*)
...
1. 集群中至少需要有一个 BackupStorageLocation。接下来的操作是针对每一个 BackupStorageLocation，在没有设置默认的 BackupStorageLocation 时，会根据 Velero server 启动命令中的信息设置一个默认	注以：在 Velero 之前版本中，Velero server 启动命令时可以设置默认的 BackupStorageLocation，在 2.0 之后废弃，改用 velero backup-location set --default 的方式，而在 Controller 中仍然保留了这段逻辑，用于向后兼容*
