...
删除备份任务时，根据 `bsl name`，`--all` 或者 `--selector` 获取到集群中的 BackupStorageLocation 资源并删除。<br>*删除 BackupStorageLocation 时，并不会删除集群中相关联的 Backup。*
...
对象不支持手动创建，而是在备份流程中，由 Backup Controller 会调用 **ensureRepo**，针对每一个卷命名空间，创建一个该对象。
...
1. 集群中至少需要有一个 BackupStorageLocation。接下来的操作是针对每一个 BackupStorageLocation，在没有设置默认的 BackupStorageLocation 时，会根据 Velero server 启动命令中的信息设置一个默认 <br>*在 Velero 之前版本中，Velero server 启动命令时可以设置默认的 BackupStorageLocation，在 2.0 之后废弃，改用 velero backup-location set --default 的方式，而在 Controller 中仍然保留了这段逻辑，用于向后兼容*
...
3. 通过 ObjectStore 的 ListCommonPrefixes 接口判断 BackupStorageLocation 是否可用<br>*具体规则是去除 prefix 目录层级，获取下一个目录层级，该目录层级应为 backups、restores、restic、metadata 和 plugins 中之一。详细逻辑参考 [TestIsValid](https://github.com/vmware-tanzu/velero/blob/5fe3a50bfddc2becb4c0bd5e2d3d4053a23e95d2/pkg/persistence/object_store_test.go#L70)*
...
3. 否则会进一步判断 ResticRepository 对象状态，如果状态为 Ready，则执行 `restic prune`，并更新 ResticRepository 上次维护时间信息；如果状态为 NotReady，调用 **ensureRepo** 尝试检查或初始化一个仓库，并根据调用结果更新 ResticRepository 的状态为 Ready 或者 NotReady<br>*restic prune 的执行失败并不会影响到主流程，只是会在 ResticRepository 对象中记录错误信息*
