...
如果 Istio 集群由 Isio Operator 创建，那么 Istio Operator 会维护 IstioOperator 对象
...
### 创建 Gateway 与默认的 VirtualService
 ...
 为了保证服务流量可以进入 IngressGateway 的 service，需要创建 Gateway 资源，同时创建一个默认的 VirtualService
...
创建的 Gateway，本质上是将访问所有 path 的流量，转入 istio-ingressgateway Service 的 80 端口
...
创建的 VirtualService，本质上是将匹配到的路由定向到 productpage Service 的 9080 端口
...
此时 Bookinfo 的 VirtualService 只是将流量转发到 productpage 上，而 productpage 只有 v1 版本，其他 DestinationRule 只是创建，未被引用
