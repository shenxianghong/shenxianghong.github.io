...
9. 如果备份规格中指定了备份集群级别资源（IncludeClusterResources），则额外备份 CRD 资源<br>*这里的 CRD 资源其实是有限制条件的，就是仅处理已经备份了与 CRD 相关的资源时，才会备份对应的 CRD*
...
5. 针对类型是 Pod 的资源，在 spec.Volumes 中获取需要借助 Restic 备份的卷<br>*由于 PV 和 PVC 是 1 对 1，PVC 和 Pod 是 1 对 n 的关系，所以在这里通过跟踪 PVC 的备份情况即可判断卷是否被备份过，备份过的卷，会在相关的 tracker 中以 PVC 为 key 作为记录*
