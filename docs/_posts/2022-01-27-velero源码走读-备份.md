...
- 必须包含一个 Backup 名称，除非指定了 --from-schedule
- 如果指定 --storage-location，则在集群中必须存在
- 如果指定 --volume-snapshot-locations，则在集群中必须存在
...
*如果 Backup 是基于定时任务（Schedule）创建的，则忽略其他所有的 filter 信息，以 Schedule 规格为准，除此之外，Backup 的名称也可以不指定，默认为 schedule-timestamp*。
...
1. 获取到 Backup 的删除事件
2. 获取到卷备份的信息和 CSI 快照信息
3. 将以上信息和 Backup 的元信息、规格、状态、删除事件详情、CSI 卷快照以及 Pod 卷数据备份等汇总作为描述信息格式化输出
   - 在开启 --details 时，会通过 StorageProvider 下载 download target kind 为 BackupResourceList 的信息
...
1. 根据命令行参数，构建 DownloadRequest 对象，download target kind 为 BackupContents，下发至集群中创建，后续由 DownloadRequest Controller 负责维护
2. 阻塞直至 DownloadRequest 的 DownloadURL 被设置，下载文件至 --output 指定的位置
...
1. 根据命令行参数，构建 DownloadRequest 对象，download target kind 为 BackupLog，下发至集群中创建，后续由 DownloadRequest Controller 负责维护
...
对象不支持手动创建，而是在备份流程中，由 Backup Controller 会调用 **backupPodVolumes**，针对每一个 Pod 卷，创建一个该对象。
...
*<u>pkg/controller/backup_controller.go</u>*<br>*<u>pkg/backup/backup.go</u>*c
...
4. 通过 StorageProvider 的 BackupExists 方法判断远端存储中是否有同名备份，如果存在则设置 Backup 状态为 Failed，本次备份失败，否则调用 **Backup** 进行准备与备份
...
7. 重新获取 BackupStorageLocation，避免长时间备份中认证信息变动， 调用 StorageProvider 的 PutBackup 接口将备份信息上传至 BackupStorageLocation 中，具体文件的对应关系如下：
...
7. 针对类型是 PV 的 item，调用 [takePVSnapshot](https://github.com/vmware-tanzu/velero/blob/5fe3a50bfddc2becb4c0bd5e2d3d4053a23e95d2/pkg/backup/item_backupper.go#L404)（TODO），忽略已经被 Restic 备份的 PV，初始化 SnapshotProvider，通过一系列的接口实现，完成卷快照的操作，并将 snapshot 记录在 request 的 VolumeSnapshots 中<br>*在 Velero 开启 CSI 特性时，需要额外加载一个 Plugin（https://github.com/vmware-tanzu/velero-plugin-for-csi），该 plugin 就是 volumeshotter 类型。因此在这步时，便会对 PV 做快照操作*
...
8. 调用 StorageProvider 的 GetBackupVolumeSnapshots 方法获取 BackupVolumeSnapshotsKey（也就是 backup-volumesnapshots.json.gz） 的内容，调用 VolumeSnapshotter 的 DeleteSnapshot 接口删除 PV 快照信息
...
10. 调用 StorageProvider 的 DeleteBackup 接口删除 Storage Provider 中 Backup 所在的目录
...
3. 调用 StorageProvider 的 ListBackups 方法获取所有的 Backup
4. 获取集群中所有的 Backup
