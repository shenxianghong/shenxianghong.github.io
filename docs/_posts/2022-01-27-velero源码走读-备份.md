...
*创建定时备份时并不会校验 schedule 表达式的合法性，而是交给 `Schedule Controller` 作后续处理。*
...
2. 仅处理状态是`空`或者 `New` 的 Backup 对象
3. 通过 **prepareBackupRequest** 进行赋值和校验的工作，并根据校验的结果设置 request 的状态为 FailedValidation 或者 InProgress
4. 将 request.Backup 信息更新至 original，更新返回的结果作为新的 original，并深拷贝一份出来作为 request.Backup 供后续流程使用，后续步骤的函数操作对象仍为 request
5. 过滤掉校验失败的 backup，调用 **runBackup**，进行执行和上传备份数据的流程
6. 步骤 5 的执行结果决定了备份是否顺利完成，如果有错误返回，则记录 request 状态为 Failed，并根据具体的状态，上报指标至 Prometheus 中
7. 以上的操作均是基于 request，最后会将 request.Backup 更新至 orginal 中，完成集群中 Backup 对象更新
...
7. 重新获取 BackupStorageLocation，避免长时间备份中认证信息变动， 借助 plugin 的 `putObject` 接口将备份信息上传至 BackupStorageLocation 中，具体文件的对应关系如下：
   ...
   - VolumeSnapshots 是相比于 CSI 而言属于 Velero 原生的卷快照，用于描述一个 PV 快照的信息，本身作为 backup 的一部分，数据最终会由对应的 VolumeSnapshotter 处理
...
3. 设置 request 的 resource & namespace 的 included & excluded、resources hook 以及 resolve action（BackupItemAction）
   ...
   - 通过 BackupItemAction 的 AppliesTo 接口获取到 labelSelector，用于备份阶段的过滤筛选
...
4. 执行 pre hook 动作（只有类型是 Pod 的 item 才会真正的处理 pre hook），hook 作用的对象需要满足 BackupItemAction 中定义的 applyTo 接口的标签选择，hook command 的执行是通过 K8s rest API exec 实现
...
6. 调用 BackupItemAction 的 execute 接口，进行额外的操作，如更新等，此后的流程基于接口返回的对象继续操作，执行失败时，会执行 post hook 动作

7. 针对类型是 PV 的 item，调用 [takePVSnapshot](https://github.com/vmware-tanzu/velero/blob/5fe3a50bfddc2becb4c0bd5e2d3d4053a23e95d2/pkg/backup/item_backupper.go#L404)，忽略已经被 Restic 备份的 PV，初始化 `VolumeSnapshotter`，通过一系列的接口实现，完成卷快照的操作，并将 snapshot 记录在 request 的 VolumeSnapshots 中
...
2. 过滤掉 hostPath 类型的卷，对其余的合法卷会创建对应的 `PodVolumeBackup` 对象，其中 PodVolumeBackup 中会设置 velero.io/backup-name 标签以及 PVC 的 UID 信息
...
2. 监听 PodVolumeBackup 资源的 Add 和 Update 事件：将状态是空或者 New 并且位于`当前节点`的 PodVolumeBackup 资源以 key （namespace/name） 的形式加入 Generic Controller 的 queue 中

   *PodVolumeBackup Controller 运行在 DaemonSet 形式的 Restic 服务中，挂载所在节点的 Pod 卷，因此仅处理本节点的 PodVolumeBackup*
...
2. 仅处理状态为`空`或者 `New` 的 PodVolumeBackup 对象，调用 **processBackup**，执行卷数据的备份
...
2. 仅处理状态为`空`、`New` 或者 `Enabled` 的 Schedule 对象
...
<u>*pkg/controller/backup_deletion_controller.go*</u>
<u>*internal/delete/delete_item_action_handler.go*</u>
...
2. 删除集群中针对该 Backup 的其余 DeleteBackupRequest 对象
...
8. 调用 ObjectStore 的 `ObjectExists` 和 `GetObject` 接口获取 BackupVolumeSnapshotsKey（也就是 `<backup>-volumesnapshots.json.gz`） 的内容，调用 VolumeSnapshotter 的 DeleteSnapshot 接口删除 PV 快照信息

9. 获取到与 Backup 相关联的 PodVolumeBackup 信息，进而获取到 Restic 的 snapshots，执行 `restic forget` 删除

   快照

   *创建备份的时候，会根据 Pod 卷创建 PodVolumeBackup 对象，并会设置 velero.io/backup-name 标签*

10. 调用 ObjectStore 的 `ListObjects` 和 `DeleteObject` 接口删除 Storage Provider 中 Backup 所在的目录

11. 如果 Velero 开启了 CSI 特性，那么也会删除与 Backup 相关联的 VolumeSnapshot 和 VolumeSnapshotContent 对象

    *删除之前会将 VolumeSnapshotContent 回收策略置为 Delete*

12. 调用 ObjectStore 的 `ListObjects` 和 `DeleteObject` 方法，删除 BackupStorageLocation 上基于该 Backup 创建的 Restore 文件，删除基于该 Backup 创建的 Restore 对象

13. 如果以上步骤均无错误返回，则删除集群中相关的 Backup 对象

14. 删除结果上报至 Prometheus，更新 DeleteBackupRequest 状态设为 Processed，并设置错误信息
...
3. 遍历 Content 中的对象，执行 DeleteItemAction 中声明的动作
...
1. 获取集群中所有的 DeleteBackupRequest 对象
2. 将状态已经处于 Processed，并且 age 超过 24 小时的 DeleteBackupRequest 对象删除
...
2. 遍历所有的步骤 1 中的 BackupStorageLocation 列表，如果 `spec.BackupSyncPeriod` 等于 0 代表该 BackupStorageLocation 不作同步操作，否则判断其是否到达下次同步时间
3. 调用 ObjectStore 的 `ListBackups` 方法获取所有的 Backup
4. 获取集群中所有的 Backup
5. 获取在 BackupStorageLocation 中但是不在集群中的 Backup，这些 Backup 就是待同步的对象
6. 针对每一个待同步的 Backup，调用 ObjectStore 的 `GetBackupMetadata` 方法，获取 Metadata 文件（即 velero-backup.json），解析内容得到 Backup 对象
7. 设置 Backup 的 namespace、resourceVersion、storageLocation 和 label 等信息，并在集群中创建
8. 调用 ObjectStore 的 `GetPodVolumeBackups` 方法获取与该 Backup 相关的 PodVolumeBackup
9. 设置 PodVolumeBackup 的 namespace、resourceVersion、ownerReferences 和 label 等信息，并在集群中创建
10. 如果 Velero 开启了 CSI 特性，通过 ObjectStore 的 `GetCSIVolumeSnapshotContents` 方法获取与该 Backup 相关的VolumeSnapshotContents
11. 设置 VolumeSnapshotContent 的 resourceVersion 等信息，并在集群中创建
12. 删除孤儿 Backup，也就是在集群中存在，但是在 BackupStorageLocation 中不存在的并且状态为 Completed 的 Backup
13. 更新集群中该 BackupStorageLocation 的上次同步时间
