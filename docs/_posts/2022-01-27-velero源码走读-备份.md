...
7. 针对类型是 PV 的 item，调用 [takePVSnapshot](https://github.com/vmware-tanzu/velero/blob/5fe3a50bfddc2becb4c0bd5e2d3d4053a23e95d2/pkg/backup/item_backupper.go#L404)（TODO），忽略已经被 Restic 备份的 PV，初始化 `VolumeSnapshotter`，通过一系列的接口实现，完成卷快照的操作，并将 snapshot 记录在 request 的 VolumeSnapshots 中<br>*在 Velero 开启 CSI 特性时，需要额外加载一个 Plugin（https://github.com/vmware-tanzu/velero-plugin-for-csi），该 plugin 就是 volumeshotter 类型。因此在这步时，便会对 PV 做快照操作*
8. 针对类型是 Pod 的 item，调用 **BackupPodVolumes**，借助 Restic 能力实现对 Pod 卷数据的备份
9. 执行 post hook 动作
10. 在 resources 目录下写入备份的资源信息，资源会根据 kind、 namepace 等信息归类，文件内容为 item 的 unstructured.Unstructured 形式，以 json 格式存储，文件源自上层调用传入的 Content 文件<br>*文件上层调用传入的 Content 文件*
...
4. 如果 BackupStorageLocation 有 caCert 证书信息，会将其临时持久化到磁盘中，供 Restic 认证使用<br>*因为本质上，Restic 的 repo 和 Velero 的 BackupStorageLocation 为同一个*
