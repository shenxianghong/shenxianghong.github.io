...
| --default-restic-prune-frequency                             | 对 Restic repo 执行 restic prune 的默认周期                  |
...
| --restore-only                                               | 是否以 Restore-Only 形式运行服务<br>*参数已经预计废弃，取而代之的是将 BackupStorageLocation 设为 ReadOnly* |
...
| --show-labels          | 在 table 样式下，最后一栏展示标签信息                        |
| --validation-frequency | Backup 同步的时间间隔，默认为 1 分钟，设置为 0 时表示禁用同步 |
