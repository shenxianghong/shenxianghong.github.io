---
layout: post
title:  "[ Velero ] 2 Restic"
date: 2021-06-27
excerpt: "Velero 与 Restic 集成实现对卷数据的备份与恢复"
project: true
tag:
- Cloud Native
- Kubernetes
...
相比于之前的部署结果，在指定 `--use-restic` 之后，额外部署了 DaemonSet 类型的 Restic 服务；在指定了 `--default-volumes-to-restic` 之后，Velero 的启动参数中会启动 feature 特性。
...
# 模拟故障
$ rm -rf /tmp/example/* && kubectl delete pod test-pod && kubectl delete pvc test-claim && kubectl delete pv test-pv && kubectl apply -f pv.yaml
...
针对每一个待备份卷数据的 Pod 所在的 namespace，velero 会创建一个和 namespace 对应的 resticrepositories，如果对应 namespace 的 resticrepositories 存在，则不会重复创建，命名方式为 `<namespace>-<backupstoragelocation>-<id>`。
...
代表 Pod volume 的备份任务，每有一个待备份的 Pod 卷，Velero 会创建一个和 backup 对应的 podvolumebackups，由于该 CR 是和 backup 对应且 backup 名称唯一，所以针对相同的 Pod 卷的多次备份，会创建多个 podvolumebackups，命名方式为 `<backup>-<id>`，各节点上的 restic daemonset  controller 会根据 podvolumebackups 指定 `restic backup` 命令。
...
代表 Pod volume 的恢复任务，每有一个待恢复的 Pod 卷，Velero 会创建一个和 restore 对应的 podvolumerestores，由于该 CR 是和 restore 对应且 restore 名称唯一，所以针对相同的 Pod 卷建多个 podvolumerestores，命名方式为 `<restore>-<id>`，各节点上的 restic daemonset  controller 会根据 podvolumerestores 执行 `restic restore` 命令。
...
Velero 在开启 Restic 对 Pod volume 备份时，根据两种条件获取待备份的卷信息：

**Velero args**

在开启 `default-volumes-to-restic` 时，默认所有备份均使用 restic 备份所有的 Pod 卷。
...
2. 并非所有的 in-tree volume 均会备份，例如以下卷类型不会参与备份：
...
与此同时，每个节点的 restic controller 会有一个 `/var/lib/kubelet/pods` 的 hostPath 卷挂载用来访问 Pod 的卷数据，通过访问该 hostPath 卷，获取到待备份的 Pod 卷数据后，执行 `restic backup`，并根据实际情况将 podvolumebackups 状态设置为 completed 或者 failed。

在每一个 podvolumebackups 完成时，Velero 会将信息添加到 `<backup-name>-podvolumebackups.json.gz` 文件中，备份完成时，汇总候上传到后端存储中，该文件中包含本次备份所有的 podvolumebackups  信息。
...
Velero 向每一个待恢复卷数据的 Pod 注入一个 init 容器，这个程序会一直等待，直到在每一个恢复的卷中找到一个位于 .velero 下的文件，该文件的名称是恢复任务的 UUID，即 Pod 完成所有卷数据的恢复。
...
与此同时，每个节点的 restic controller 会有一个 `/var/lib/kubelet/pods` 的 hostPath 卷挂载用来访问 Pod 的卷数据，等待 Pod 运行 init 容器，通过访问该 hostPath 卷，获取到待备份的 Pod 卷数据后，执行 `restic restore`，成功之后，将文件写入 Pod 卷中的 .velero 子目录中，名称是此 Pod 卷还原的 UUID，并根据实际情况将 PodVolumeRestore 状态设置为 completed 或者 failed。
...
默认情况下，Velero 会备份静态 PV，如 local pv、nfs pv 等，但是在恢复的时候，如果待恢复的对象中包含使用该 PV 的 Pod 时，Velero 并不会恢复 PV，而是默认由 StorageClass 动态供应创建 PV，此时的 PVC 会处于 pending 状态（由于不存在 PV），Pod 也会处于 pending 状态（由于 PVC pending），此时的 restore 任务会等待直至超时，目前来看 Velero 社区并没有将此视为 bug，因为在使用层面进行处理，方案的核心思路是如果恢复的 Pod 使用静态的 PV 时，需要确保恢复流程执行之前，存在一个可以满足 PVC 的 PV，例如：
