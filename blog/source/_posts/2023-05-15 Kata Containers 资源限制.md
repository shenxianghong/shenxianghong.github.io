...
为什么不直接将 sandbox、shim 等进程添加到 /kubepods cgroup？

Kata shim 实现 per-sandbox cgroup （即每一个 sandbox 都有一个 /kata\_\<sandboxID\>）来支持 Docker 场景。尽管 Docker 没有 Pod 的概念，但 Kata Containers 仍然创建了一个 sandbox 来支持 Docker 实现的无 Pod、单一容器用例（即 single_container）。为了简化 Docker 场景，Kata Containers 选择一个独立的 /kata\_\<sandboxID\>，而不是构建容器和 sandbox 之间的 cgroup 映射关系。

### 优点

将 Kata Containers 所有进程放置在适当大小的 Pod cgroup 中是理想的，并且可以简化 Kata Containers 的实施。还有助于收集准确的统计数据并防止 Kata 工作负载产生的近邻干扰（noisy neighbor）。

**Pod 资源统计**

如果 Kata 使用者想知道 host 上的资源使用情况，它可以从 Pod cgroup 中获取统计信息。cgroup 统计数据中包括 Kata 的额外开销。提供了在 Pod 级别和容器级别收集使用静态信息的能力。

**更好的 host 资源隔离**

因为 Kata 运行时会将所有 Kata 进程放在 Pod cgroup 中，所以使用者对 Pod cgroup 设置的资源限制将影响 host 中属于 Kata sandbox 的所有进程（例如 qemu-system、virtiofsd 等）。可以改善 host 中的隔离，防止 Kata 产生的近邻干扰（noisy neighbor）。

## sandbox_cgroup_only = false

如果提供给 Kata 的 cgroup 大小不合适，Kata 组件将消耗实际容器工作负载期望看到和使用的资源。这可能会导致不稳定和性能下降。

为避免这种情况，Kata Containers 创建了一个不受约束的 cgroup，并将所有与工作负载无关的进程（除 vCPU 线程外的任何进程）移至其中。这个 cgroup 的名称是 /kata_overhead，并针对 Kata Containers 创建的每个 sandbox 均在 /kata\_overhead 下创建一个对应的子 cgroup。

Kata Containers 不对 overhead cgroup 添加任何约束或限制，因此可以

- 预先创建一个预期大小的 /kata_overhead cgroup，Kata Containers 不会再额外创建，而是将所有与工作负载无关的进程移动到其中
- 让 Kata Containers 创建 /kata_overhead cgroup，让它不受约束或事后调整它的大小
...
当 sandbox_cgroup_only 被禁用时，Kata shim 将在 /kubepods cgroup（在 Kubernetes 场景中，/kubepods 为 Pod 专用的 cgroup）下创建名为 /kata\_\<sandboxID\> 的子 cgroup，并在 /kata_overhead 的 cgroup 下创建一个名为 /\<sandboxID\> 的子 cgroup。

与启用 sandbox_cgroup_only 时不同，Kata shim 将自身加入到 /kata_overhead cgroup 中，然后在 vCPU 线程创建时将其移动到 /kata\_\<sandboxID\> cgroup。除 vCPU 线程外，所有 Kata 进程和线程都将在 /kata_overhead cgroup 下运行。

在禁用 sandbox_cgroup_only 的情况下，Kata Containers 假定 Pod cgroup 的大小仅能满足容器工作负载进程。VMM 创建的 vCPU 线程是唯一在 /kubepods cgroup 下运行的线程，降低了 VMM、Kata shim 和 I/O 线程 OOM 的风险。
